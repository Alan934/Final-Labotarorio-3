...
<head>
    ...
    <title>Agregar-Whiskys-Admin-Barpincho</title>
...
if(isset($_POST['confirmar'])){
    if(isset($_FILES['archivo']['name'])){
        if(file_exists($_FILES['archivo']['tmp_name']) && strlen($_POST['nombre']) >= 1 && strlen($_POST['precio']) >= 1 && strlen($_POST['capacidad']) >= 1){
            $url = 'imagenes/productos/whiskys/'.$_FILES['archivo']['name'];
            $nombre = trim($_POST['nombre']);
            $precio = trim($_POST['precio']);
            $capacidad = trim($_POST['capacidad']);
            if(move_uploaded_file($_FILES['archivo']['tmp_name'], '../../../imagenes/productos/whiskys/'.$_FILES['archivo']['name'])){
                $query = $conex->query("INSERT INTO whiskys(nombre, url, precio, capacidad) VALUES ('".$nombre."', '".$url."', '".$precio."', '".$capacidad."')");
                $_SESSION['mensajeWhiskys'] = '¡Whisky Agregado¡';
                $_SESSION['mensajeColorWhiskys'] = 'success'; 
            }
        }else{
            $_SESSION['mensajeWhiskys'] = '¡Complete todos los campos¡';
            $_SESSION['mensajeColorWhiskys'] = 'info'; 
        }
    }
}
?>
...
<body class="bodyAgregarProducto">
    ...
    <main class="form-signin w-50 m-auto">
        ...
        <div class="w-100 d-flex flex-wrap justify-content-center text-center">
            <?php if(isset($_SESSION['mensajeWhiskys'])){ ?>
                <div class="alert alert-<?= $_SESSION['mensajeColorWhiskys']; ?> alert-dismissible fade show" role="alert">
                    <?= $_SESSION['mensajeWhiskys']; ?>
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                </div>
            <?php } ?>
        </div>
        <form action="agregarWhiskys.php" method="POST" enctype="multipart/form-data">
            <h1 class="h3 mb-3 text-warning text-center fw-bold">AGREGAR WHISKYS</h1>
